...
function getAverage(value1, value2) {
  if (!Number.isFinite(value1) || !Number.isFinite(value2)) {
    throw new Error('Invalid input: one of the values is not a valid number.');
  }

  const average = value1 / 2 + value2 / 2;

  if (!Number.isFinite(average)) {
    throw new Error('Calculation error: result is not a valid finite number.');
  }
...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    throw new Error('Coefficient "a" cannot be 0 for a linear equation');
  }
  return -b / a;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const pointsMult = x1 * x2 + y1 * y2;

  const magU = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magV = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(pointsMult / (magU * magV));
}
...
function getSumOfNumbers(x1, x2, x3) {
  if (!Number.isFinite(x1) || !Number.isFinite(x2) || !Number.isFinite(x3)) {
    throw new Error('All inputs must be valid finite numbers.');
  }

  const result = (x1 + x2 + x3).toFixed(10);
...
function getHypotenuse(a, b) {
  ...
  if (!Number.isFinite(hypotenuse)) {
    return 'Invalid calculation, result is too large';
  }
